...
Fetched automatically by `fetch_events.py` (same folder) from OpenLigaDB (free, no key) —
see "Data source notes" in this folder's `CLAUDE.md`. Run it after a match to populate the
file below, then run `01-recap-graphics/generate_recap.py`. Nothing here is ever
fabricated or guessed: it's always real data from OpenLigaDB, or a manual edit you made
yourself — `fetch_events.py` asks before overwriting a file that already exists.
...
- `competition` comes straight from whichever OpenLigaDB competition the match was fetched
  ...
  `"Friendly"` — see "Data source notes" in this folder's `CLAUDE.md`.
- Featured player photo is picked automatically from `01-recap-graphics/players/` based on
  the match result — see `01-recap-graphics/players/README.md` for exactly how. Add
