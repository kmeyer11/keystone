...
- `src/fetch_events.py` — fetches goal-scorer data for the latest played match from OpenLigaDB (`api.openligadb.de`, free, no key, German-football-specific) and writes it to `data/matches/<date>_<opponent>.json` in the schema `match_events.py`/`generate_recap.py` expect. Prints the fetched JSON before writing and asks for confirmation if the file already exists (so a manual edit already made isn't silently clobbered). Run from the repo root (`python src/fetch_events.py`) before `generate_recap.py`.
...
`openfootball/football.json` (`src/fetch_matches.py`) only has fixtures and final/half-time scores — no scorers, no match events, no lineups. `api.openligadb.de` (`src/fetch_events.py`) fills that gap for free, no key required: per-goal scorer name, minute, and own-goal/penalty/overtime flags for Bundesliga matches, as soon as OpenLigaDB itself marks the match `matchIsFinished`. Run `fetch_events.py` after a match to populate `data/matches/*.json`, then `generate_recap.py`. It still only ever writes real API data — never fabricate scorer/event data by hand to fill a gap; either a real source has it, or it isn't shown. A `data/matches/*.json` file can still be hand-edited afterward (e.g. to fix a scorer-name quirk, or set `featured_player_photo`) — `fetch_events.py` asks before overwriting an existing file. Same principle applies to the future highlight-edit feature.
